/**
 * API base URL:
 * - Production: set via NEXT_PUBLIC_API_URL env var (e.g. https://api.cap-track.cebo.tech)
...
class ApiClient {
  ...
  async revokeInvitation(invitationId: string) {
    const res = await this.fetch(this.clubUrl(`/invitations/${invitationId}`), {
      method: "DELETE",
    });
    return res.json();
  }
  ...
  async acceptInvitation(token: string) {
    const res = await this.fetch(`${API_URL}/v1/invitations/${token}/accept`, {
      method: "POST",
    });
  ...
  async getDashboard(ageCategory?: string) {
    ...
    if (ageCategory)
      params.push(`age_category=${encodeURIComponent(ageCategory)}`);
    const res = await this.fetch(
      this.clubUrl(`/dashboard?${params.join("&")}`),
    );
  ...
  async setQuarter(quarter: number) {
    return this.resilientWrite(this.clubUrl("/settings/quarter"), "PUT", {
      quarter,
    });
  }
  ...
  async setPlayerAgeCategories(playerId: string, categories: string[]) {
    const res = await this.fetch(
      this.clubUrl(`/players/${playerId}/age-categories`),
      {
        method: "PUT",
  ...
  async deleteAgeCategory(categoryId: string) {
    const res = await this.fetch(
      this.clubUrl(`/age-categories/${categoryId}`),
      {
        method: "DELETE",
      },
    );
  ...
  async getPreviousRoster(matchId: string) {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/previous-roster`),
    );
    return res.json();
  }
  ...
  async updateScore(
    ...
    return this.resilientWrite(
      this.clubUrl(`/matches/${matchId}/scores`),
      "POST",
      {
        quarter,
        my_score: myScore,
  ...
  private async resilientWrite(url: string, method: string, payload?: any) {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        getOfflineQueue().addToQueueWithLocalId(
          url,
          { method, headers, body },
          localId,
        );
  ...
  async getEvents(matchId: string, limit = 20) {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/events?limit=${limit}`),
    );
    return res.json();
  }
  ...
  async getPlaytime(matchId: string) {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/playtime?t=${Date.now()}`),
    );
  ...
  async recordSubstitution(
    ...
    return this.resilientWrite(
      this.clubUrl(`/matches/${matchId}/substitutions`),
      "POST",
      {
        player_id: playerId,
        direction,
  ...
  async undoEvent(windowMinutes = 3) {
    return this.resilientWrite(this.clubUrl("/events/undo"), "POST", {
  ...
  async getMatchStats(matchId: string) {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/stats?t=${Date.now()}`),
    );
    return res.json();
  }
  ...
  async uploadVoiceNote(
    ...
    if (opts.duration_s != null)
      fd.append("duration_s", String(opts.duration_s));
  ...
  async getVoiceNotes(matchId: string) {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/voice-notes`),
    );
    if (!res.ok) return [];
    return res.json();
  }

  async getVoiceNoteAudioUrl(
    matchId: string,
    noteId: string,
  ): Promise<string | null> {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/voice-notes/${noteId}/audio`),
    );
  ...
  async deleteVoiceNote(matchId: string, noteId: string) {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/voice-notes/${noteId}`),
      { method: "DELETE" },
    );
    return res.json();
  }
  ...
  async getMvp(matchId: string) {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/mvp?t=${Date.now()}`),
    );
    if (!res.ok) return null;
    return res.json();
  }
  ...
  async getEventVideoUrl(matchId: string, eventId: string) {
    const res = await this.fetch(
      this.clubUrl(`/matches/${matchId}/events/${eventId}/video-url`),
    );
    if (!res.ok) return null;
    return res.json();
  }
