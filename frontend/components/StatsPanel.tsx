...
export default function StatsPanel({ state, showToast }: Props) {
  ...
  if (!stats) return <div className="wrap"><div className="muted">Wybierz mecz aby zobaczyć statystyki</div></div>;
    ...
    <div className="wrap">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        {['all', '1', '2', '3', '4'].map(q => (
          <button
            key={q}
            className={`btn small${quarter === q ? ' primary' : ''}`}
            onClick={() => setQuarter(q)}
          >
            {q === 'all' ? 'Wszystkie' : `Q${q}`}
          </button>
        ))}
        <button className="btn small" onClick={loadStats} style={{ marginLeft: 'auto' }}>
          Odśwież
        </button>
      </div>
        ...
        <table className="stats-table">
          <thead>
            <tr>
              <th>Statystyka</th>
              {players.map((p: any) => (
                <th key={p.player_id}>#{p.number} {p.name}</th>
              ))}
              ...
              <>
                ...
                {group.flags.map(flag => (
                  <tr key={flag}>
                    ...
                    <td style={{ fontWeight: 700, background: 'var(--bg-hover)' }}>
                      {totals[flag] || '·'}
                    </td>
                  </tr>
                ))}
